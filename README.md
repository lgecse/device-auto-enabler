...
Edit `C:\ProgramData\DeviceAutoEnabler\config.json` (requires Administrator). The service watches this file and **hot-reloads** on save. If you save an invalid file, it is rejected and the last-good configuration is kept (the rejection is logged), so a typo never crash-loops the service.
